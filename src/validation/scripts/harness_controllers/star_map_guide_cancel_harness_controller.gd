...
## Validation harness for star map guide cancellation.
## Tests: same planet click cancels, explicit cancel_guide_mode() cancels.
...
func _physics_process(_delta: float) -> void:
	...
	match _step:
		...
		60:
			# Click earth again to cancel guide mode (same planet click)
			star_map._on_planet_clicked("earth")
		80:
			# Click mars to enter guide mode again
			star_map._on_planet_clicked("mars")
		100:
			# Explicitly cancel guide mode
			star_map.cancel_guide_mode()
